...
# ── SCRUM-123: Validación de rangos de precio unitario ───────────────────────
# Ningún precio unitario en el desglose puede estar fuera de este rango.
# Si está fuera, se descarta y el insumo queda marcado como sin_cotizacion.
PRECIO_MIN_CLP = 1.0         # mínimo absoluto: 1 CLP
PRECIO_MAX_CLP = 2_000_000.0  # máximo absoluto: 2.000.000 CLP

# ── SCRUM-123: Conversiones explícitas (sin magic numbers dispersos) ─────────
# Factor para convertir precio "por kg" a precio "por saco 25 kg"
KG_POR_SACO_CEMENTO = 25.0
# Factor para convertir m³ (hormigón dosificado) a m² de losa a 10 cm
LOSA_ESPESOR_M = 0.10  # alias semántico de ESPESOR_LOSA_HA, para losa_hormigon
# Densidad áridos (arena/gravilla/ripio) para convertir kg → m³
DENSIDAD_ARIDOS_KG_M3 = 1600.0


def _validar_precio_rango(precio: float) -> bool:
    """SCRUM-123: Devuelve True si el precio está dentro del rango permitido."""
    return PRECIO_MIN_CLP <= precio <= PRECIO_MAX_CLP
...
def _calcular_insumos_material(
    ...
    # SCRUM-123: Mapa de unidad_precio declarada explícitamente en BD.
    # Proviene del campo PrecioMercado.unidad_precio. Si es NULL en la BD,
    # se usa la unidad_medida del Insumo como fallback (sin inferencia regex).
    unidad_precio_map: dict[int, str] = {}
    for pm in precios_records:
        if pm.insumo_id not in unidad_precio_map:
            val = getattr(pm, "unidad_precio", None)
            if val:
                unidad_precio_map[pm.insumo_id] = str(val).strip()
    ...
    for r, insumo in datos_rendimiento:
        ...
        if cantidad_override is not None:
            ...
            # SCRUM-123: Losa de 10 cm — conversiones explícitas con constantes declaradas.
            # kg/m³ → kg/m² usando espesor LOSA_ESPESOR_M (10 cm = 0.10 m)
            factor_ajustado = float(r.factor_multiplicador)
            factor_ajustado *= LOSA_ESPESOR_M  # kg/m³ × espesor(m) = kg/m²
            if any(k in nombre_l for k in ["arena", "gravilla", "ripio"]):
                # áridos: kg/m² → m³/m² dividiendo por densidad aparente
                factor_ajustado /= DENSIDAD_ARIDOS_KG_M3
            if "cemento" in nombre_l:
                # cemento: kg/m² → sacos de 25 kg
                factor_ajustado /= KG_POR_SACO_CEMENTO
        ...
        subt = None
        # SCRUM-123: unidad_precio declarada explícitamente en BD (sin inferencia regex).
        # Fallback al campo unidad_medida del Insumo si la columna aún es NULL.
        unidad_precio_item = unidad_precio_map.get(insumo.id) or insumo.unidad_medida or ""
        sin_cotizacion_item = precio_unit is None

        if precio_unit is not None:
            precio_unit_normalized = float(precio_unit)
            # Normalización de unidades de mano de obra: si el insumo espera HH pero el
            # producto indica precio por jornada/día, convertir a precio por HH.
            try:
            ...
            if unidad_esperada in ('HH', 'H') and precio_record is not None:
                ...
                if any(k in nombre_prod_l for k in ('jornada', 'por jornada', 'por día', 'por dia', 'día', 'dia')):
                    try:
                        ...
                        pass
            # SCRUM-123: Validación de rango. Si el precio está fuera del rango
            # permitido, se descarta y el insumo queda sin cotización.
            if not _validar_precio_rango(precio_unit_normalized):
                precio_unit_normalized = None
                precio_unit = None
                sin_cotizacion_item = True
            else:
        ...
        item = InsumoCalculado(
            ...
            # SCRUM-123: declarar unidad del precio y estado de cotización
            unidad_precio=unidad_precio_item if not sin_cotizacion_item else None,
            sin_cotizacion=sin_cotizacion_item,
        )
