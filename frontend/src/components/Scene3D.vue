...
const ensureScene = () => {
  ...
  camera = new THREE.PerspectiveCamera(60, width / Math.max(height, 1), 0.1, 3000)
  ...
  const floorMat = new THREE.MeshStandardMaterial({ color: '#1f2937', roughness: 0.9, metalness: 0.05 })
...
<template>
  <section class="scene-panel">
    <header class="scene-header">
      <h3>Renderizador Volumetrico Reactivo (SCRUM-48)</h3>
      <p>Escucha la topologia de muros y actualiza mallas 3D sin reiniciar la escena.</p>
    </header>

    <div ref="containerRef" class="scene-canvas" />

    <div class="scene-meta">
      <span>Muros: {{ topology.topologyStats.totalWalls }}</span>
      <span>Interiores: {{ topology.topologyStats.interiorCount }}</span>
      <span>Exteriores: {{ topology.topologyStats.exteriorCount }}</span>
      <span>Longitud: {{ topology.topologyStats.totalLength }}m</span>
    </div>
  </section>
</template>
