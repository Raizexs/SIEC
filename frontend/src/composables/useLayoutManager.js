...
const safeParseLayouts = () => {
  try {
    ...
    logger.error('[useLayoutManager] Error al parsear layouts de sessionStorage:', err);
    return [];
  }
};
  ...
  const persistLayouts = () => {
    try {
      ...
      try {
        ...
        logger.error('[useLayoutManager] No se pudo persistir en sessionStorage (fallback):', innerErr, { originalError: error });
      }
    }
  };
