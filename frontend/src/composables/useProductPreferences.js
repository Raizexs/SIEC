...
const dispatchPreferencesUpdated = () => {
  ...
  try {
    ...
    logger.error('[useProductPreferences] Error al serializar evento de preferencias:', err);
    window.dispatchEvent(
      new CustomEvent('siec:preferences-updated', {
...
const hydrateFromStorage = () => {
  ...
  try {
    ...
    logger.error('[useProductPreferences] Error al hidratar preferencias desde storage:', err);
...
const attachStorageSync = () => {
  ...
  window.addEventListener('storage', (event) => {
    ...
    try {
      ...
      logger.error('[useProductPreferences] Error al sincronizar preferencias desde storage event:', err);
      /* ignore: se mantiene el estado actual */
    }
  });
};
...
export function useProductPreferences() {
  ...
  const loadProductPreferences = () => {
    try {
      ...
      logger.error('[useProductPreferences] Error al cargar preferencias desde storage:', err);
      productPreferences.value = defaultProductPreferences();
    }
  };
